...
namespace Phaseproject
{
    internal class Teacher
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (FileStream fs = new FileStream(@"C:\New folder\C#\assignment.txt", FileMode.Open)) using (StreamReader sr = new StreamReader(fs))
            {
            ...
            {
                List<Teacher> listTeachers = new List<Teacher>();
                string teacherfile = @"C:\New folder\C#\assignment.txt"; 
                ...
                foreach (Teacher t in listTeachers)
                {
                    if (t.Id == id)
                    {
                        ...
                        Console.WriteLine($"{t.Id},{t.FirstName},{t.LastName},{t.CClass},{t.Section}"); break;
                    }
                }
                int count = 0;
                ...
                {
                    string s = ($"{t1.Id},{t1.FirstName},{t1.LastName},{t1.CClass},{t1.Section}"); arr[count] = s;
                    count++;
                }
                File.WriteAllLines(@"C:\New folder\C#\assignment.txt", arr);
            }
            static void create()
            {
                List<Teacher> listTeachers = new List<Teacher>();
                ...
                string UIClass = "";
                string UIsection = "";
                using (FileStream fs = new FileStream(@"C:\New folder\C#\assignment.txt", FileMode.Append)) using (StreamWriter sw = new StreamWriter(fs))
                {
                    Teacher teacher = new Teacher(); 
                    teacher.Id = UIId;
                ...
                static void delete()
                {
                    ...
                    string id;
                    Console.WriteLine("Enter the id to delete:"); id = Console.ReadLine();
                    foreach (Teacher t in listTeachers)
                    {
                        if (t.Id == id)
                        {
                ...
                static void search()
                {
                    ...
                    foreach (Teacher t in listTeachers)
                    {
                        if (t.Id == id)
                        {
                            Console.WriteLine("given {0} is present in the given file", id); Console.WriteLine($"{t.Id},{t.FirstName},{t.LastName},{t.CClass},{t.Section}"); break;
                        }
                        else
                        {
                            Console.WriteLine("enterd id is not there");
                ...
                {
                    int option;
                    Console.WriteLine("Enter Option You want to perform: "); option = Convert.ToInt32(Console.ReadLine());
                    switch (option)
                    {
                        case 1:
                            create();
